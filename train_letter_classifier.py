...
class LetterCropDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        pad = self.bbox_pad
        if self.augment:
            # Jitter crop bounds ±2 px and optional scale jitter by ±10%.
            pad = pad + random.uniform(-1.5, 1.5)
        ...
        if self.augment:
            ...
            if random.random() < 0.3:
                retina = np.clip(retina + np.random.normal(0, 0.02, retina.shape), 0, 1)
            # Small random patch erase.
            if random.random() < 0.2:
                ph = random.randint(2, 8)
                pw = random.randint(2, 8)
...
def main() -> None:
    ...
    model = LetterClassifier(retina=args.retina, base_ch=args.base_ch).to(device)
    ...
    for ep in range(args.epochs):
        ...
        print(f"  train loss={train_m['loss']:.4f} acc={train_m['acc']:.4f}  "
              f"val loss={val_m['loss']:.4f} acc={val_m['acc']:.4f}  ({dt:.1f}s)")
